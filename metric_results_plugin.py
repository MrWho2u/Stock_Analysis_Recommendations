...
def calculate_metrics (stock_return_df, required_return, ticker, bench_return_df, time_period, time_numeric, monte_stock_df, monte_bench_df, boiler_band_df):
    ...
    stock_norm_sdv = float(stock_return_df.std())
    #calcualte averge return for sharpe ratio
    stock_mean_return = float(stock_return_df.mean())
    ...
    stock_neg_sdv = float(stock_negative_returns.std())
    #convert the annual requied return into a daily return using a geometric method
    required_daily = ((1+required_return/100)**(1/252)-1)
    ...
    bench_norm_sdv = float(bench_return_df.std())
    #calcualte averge return for sharpe ratio
    bench_mean_return = float(bench_return_df.mean())
    #calculate standard deviation of negative returns for sortino ratio
    bench_negative_returns = bench_return_df[bench_return_df<0]
    bench_neg_sdv = float(bench_negative_returns.std())
    #calculate the sharpe and sortino ratios
    ...
    # Market return results language
    if bench_sharpe_ratio>stock_sharpe_ratio:
        sharpe_comp = f"Additionally, the S&P 500 has a greater Sharpe Ratio ({bench_sharpe_ratio}) than {ticker}. This indicates the the general market may have greater excess return per unit of risk."
    else:
        sharpe_comp = f"Additionally, the S&P 500 has a Sharpe Ratio ({bench_sharpe_ratio}) below that of than {ticker}. This indicates the secrutiy may have greater excess return per unit of risk than the general market."
    ...
    if (monte_stock_annual <= required_return) and (monte_bench_annual <= required_return):
        ...
        final_lang = f"According to our monte carlo simulation {ticker} was able to meet your return requirements while the genernal market was not. As such we would recommend investing in {ticker} to meet your return goals."
    elif (monte_stock_annual <= required_return) and (monte_bench_annual >= required_return):
        final_lang = f"According to our monte carlo simulation {ticker} was unable to meet your return requirements, however the genernal market did meet those requirements. As such we would recommend investing in the S&P 500 to meet your return goals."
        ...
        final_lang = f"According to our monte carlo simulation both the market and {ticker} are able to exceed your return requirement. However, the genernal market had a higher Sortino Ratio indicating that it may be able to provide a greater risk adjusted return than {ticker}."
    elif (monte_stock_annual >= required_return) and (monte_bench_annual >= required_return) and (bench_sortino_ratio < stock_sortino_ratio):
        final_lang = f"According to our monte carlo simulation both the market and {ticker} are able to exceed your return requirement. However, the {ticker} has a higher Sortino Ratio indicating that it may be able to provide a greater risk adjusted return than the market."
    else:
    ...
    return stock_sharpe_ratio, stock_sortino_ratio, bench_sharpe_ratio, bench_sortino_ratio, ratio_lang_final, sharpe_comp, return_lang, boiler_lang, final_lang